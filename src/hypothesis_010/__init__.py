...
strategy_assignment_dict = {
    ...
    "uint": (st.integers(min_value=0, max_value=4294967295), partial(to_bytes_int, sign=False)),
    "int16": (st.integers(min_value=-32768, max_value=32767), partial(to_bytes_int, sign=True)),
    "uint16": (st.integers(min_value=0, max_value=65535), partial(to_bytes_int, sign=False)), 
    "unsignedint16": (st.integers(min_value=0, max_value=65535), partial(to_bytes_int, sign=False)),
    "int64": (st.integers(
    ...
    "ushort": (st.integers(min_value=0, max_value=65535), partial(to_bytes_int, sign=False)),
    "word": (st.integers(min_value=0, max_value=65535), partial(to_bytes_int, sign=False)),
    "dword": (st.integers(
        min_value=-2147483648, max_value=2147483647
    ), partial(to_bytes_int, sign=True)),
    "long": (st.integers(
    ...
    "unsignedchar": (st.characters(
        min_codepoint=0, max_codepoint=255
    ), to_bytes_char),  # There could be chars that are below 0?
    "uchar": (st.characters(
        min_codepoint=0, max_codepoint=255
    ), to_bytes_char),
    "second": (st.integers(
        min_value=0, max_value=31
    ), partial(to_bytes_int, sign=False)),  # dostime strats
...
unassigned_strats = [] #For listing off objects that don't have a strategy in either the dictionary above or a way to compose them in parseTree below
...
def testing_script(number_of_unique_examples: int):
    ...
    ast = py010parser.parse_file(
        os.path.join(os.getcwd(), "templateRepo/Mifare4k.bt")
    )
    ...
    print(f'Finished testing, took {time_total.total_seconds():.3f} seconds to create the strategy and generate 100 unique examples')
    print(f'Time to create strategy from binary template file: {time_strat_gen.total_seconds():.3f} seconds')
    print(f'Average time to generate an example from the strategy: {avg_example_time:.3f} seconds')
    print(f'Produced {len(times)} examples, of which {len(example_collection)} were unique')
